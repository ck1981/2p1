...
While provisioning, `server.yml` will create the `users` defined in `group_vars/all`, assigning their `groups` and public SSH `keys`. The example below defines a single user.
...
If needed, you may redefine the `users` in any given `group_vars` environment file, overriding the `users` list in `group_vars/all`.
...
`server.yml` will try to connect to your server as `root`. If the connection fails, `server.yml` will try to connect as the `admin_user` defined in `group_vars/all` (default `admin`). If `root` login will be disabled on your server, it is critical for the `admin_user` to be defined in your list of `users`, with `sudo` first in this user's list of groups (see the [Security wiki](https://github.com/roots/trellis/wiki/Security)). The default definition for the `admin_user` is shown below.
